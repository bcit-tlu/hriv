...
A clean core run records terminal `state: SUCCEEDED`, `outcome: succeeded`, `core_succeeded.contract_boundary: simplified1253`, and strict bounded `last_complete_success` evidence only after confirmed child absence and successful zero-remaining cleanup. The evidence contains only run ID, completion time, recovery-set ID, source-files SHA-256, and cleanup outcome/time/count; failed runs never advance it. Failed children remain as exactly one retained environment until an operator invokes `cleanup-retained`; there is no application reaper. The cleanup command accepts no arbitrary run ID, shares the Lease/CAS state, verifies exact state-bound UID/labels/template identity, waits through partial deletion, and removes state only at zero children. An interrupted cleanup with a different Job UID may take over only after Lease expiry, old holder absence/terminal status, an unchanged sole retained/latest record, and exact validation of every observed bound or controlled-descendant resource. #1252 application/viewer validation, credential-init, Redis, tile rebuild, exporter, dashboard, custom metrics, and autonomous Job/child reapers are excluded. The CLI exposes only `run`, `cleanup-retained`, `validate-database`, and `validate-consistency`.
...
The fixed ConfigMap state is schema 1, bounded by configured `state_max_bytes` with a hard ceiling of 512 KiB, and updated with `resourceVersion` CAS. The parser hard ceiling remains two for compatibility, but operational configuration requires exactly one retained ownership record and keeps ten bounded history summaries. Runtime immutable controller/profile/policy/template ConfigMaps use atomic `-v2` names; every embedded child mount and orchestrator/cleanup mount expects `-v2`, while the fixed state ConfigMap and Lease names remain unchanged. The mounted source policy contains only version 1, the reviewed source-state SHA-256, and missing/orphan counts (each at most 256). Selection independently canonicalizes the complete backup-emitted lists, verifies their digest/counts, and immutably records the full selected state plus local profile/policy identities. Consistency recomputes and internally compares every absent row's exact ID/status/path/reason with selected canonical evidence and requires zero unexpected restored orphans. Its bounded output does not repeat missing paths; it returns `missing_count`, canonical UTF-8 `missing_sources_sha256`, `unexpected_orphan_count: 0`, source-file count/bytes/SHA-256, database count, and policy digest, remaining below 32 KiB even at 256 maximum-length entries. The Lease holder tuple is `v1|<job-uid>|<run-id>|<acquired>|<renewed>`; both holder timestamps and Lease fields are normalized to exact UTC seconds before every write and checked after API round trips. Cleanup is asynchronous: UID-preconditioned foreground deletes are issued once and reconciliation remains in `CLEANUP` until every bound and run-labelled child is absent. CNPG-added labels are permitted, and controlled CNPG Pod/Service descendants are distinguished by required labels and exact controller owner UID rather than mistaken for spoofed direct children.
