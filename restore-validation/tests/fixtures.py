...
def template_document() -> dict[str, Any]:
    def job(image: str, role: str) -> dict[str, Any]:
        ...
        if role in {"db", "consistency"}:
            volumes.extend([{"name": "credentials", "secret": {"secretName": "generated-superuser"}}, {"name": "profile", "configMap": {"name": "hriv-restore-validation-source-profile-v7"}}])
        if role == "consistency":
            volumes.append({"name": "policy", "configMap": {"name": "hriv-restore-validation-source-state-policy-v7"}})
