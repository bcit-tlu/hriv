...
class ChartTests(unittest.TestCase):
    ...
    def test_runtime_configmaps_are_atomic_v3_identities(self):
        ...
        expected = {"hriv-restore-validation-controller-v4", "hriv-restore-validation-source-profile-v4", "hriv-restore-validation-source-state-policy-v4", "hriv-restore-validation-child-templates-v4"}
    ...
    def test_chart_config_round_trip(self):
        raw = self._config_map("hriv-restore-validation-controller-v4")["data"]["config.json"]
        self.assertEqual("hriv-restore-validation", Config.parse(raw).namespace)

    def test_chart_profile_round_trip(self):
        raw = self._config_map("hriv-restore-validation-source-profile-v4")["data"]["profile.json"]
        self.assertEqual("pg-core-source", SourceProfile.parse(raw).external_cluster)

    def test_chart_policy_round_trip(self):
        raw = self._config_map("hriv-restore-validation-source-state-policy-v4")["data"]["policy.json"]
    ...
    def test_chart_current_nonzero_digest_only_policy(self):
        ...
        raw = next(item for item in documents if item and item.get("kind") == "ConfigMap" and item["metadata"]["name"] == "hriv-restore-validation-source-state-policy-v4")["data"]["policy.json"]
    ...
    def test_chart_templates_round_trip(self):
        profile_raw = self._config_map("hriv-restore-validation-source-profile-v4")["data"]["profile.json"]
        raw = self._config_map("hriv-restore-validation-child-templates-v4")["data"]["templates.yaml"]
    ...
    def test_operational_cronjobs_proxy_and_fixed_policy(self):
        ...
        child_config = next(item for item in docs if item.get("kind") == "ConfigMap" and item["metadata"]["name"] == "hriv-restore-validation-child-templates-v4")
