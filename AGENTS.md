...
- Do not edit `db/init.sql` or `charts/backend/templates/configmap-initdb.yaml` for new schema changes — they are now a legacy-bootstrap path only.
- Add a SQLAlchemy model change in `backend/app/models.py`, then generate an Alembic revision (see above) and commit the migration alongside the model change.
