...
- HRIV is `MPL-2.0`; each distributed component carries a generated
  `THIRD-PARTY-LICENSES.txt` acknowledging its bundled dependencies.
- **When you add, remove, or bump a runtime dependency, regenerate and commit
  the affected notices file** (see the README **License** section). Frontend:
  `cd frontend && npm run licenses:generate`. Backend/backup:
  `poetry run python scripts/generate_third_party_licenses.py`.
- CI fails (`frontend-checks`) if `frontend/public/THIRD-PARTY-LICENSES.txt`
  drifts from the production dependency tree.
