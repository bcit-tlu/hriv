...
calling mutation capabilities. Never select by blob mtime or ambiguous prefix. Assert that successful selection and stateless restore both emit canonical UTC `capture_started_at`, uppercase 24-hex `wal_fence_file`, a supported power-of-two `wal_segment_size_bytes`, `wal_fence_committed_at`, and `wal_fence_archived_at`, and that the first eight fence hex digits equal `target_timeline`. Database validation must report a promoted `timeline` greater than `target_tli`, `timeline_parent == target_tli`, and `timeline_switchpoint >= target_lsn`; numeric descendants already in the archive must not force rejection. Failed machine commands must emit exactly `schema_version`, `operation`, `success:false`, and bounded `failure_code` so the #1251 controller can preserve the child code.
