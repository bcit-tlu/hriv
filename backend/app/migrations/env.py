...
async def run_async_migrations() -> None:
    ...
    # ``settings.database_url`` is always populated (pydantic-settings default
    # + ``DATABASE_URL`` env override), so we wire it directly into the
    # Alembic ini section and ignore ``sqlalchemy.url`` in ``alembic.ini``.
