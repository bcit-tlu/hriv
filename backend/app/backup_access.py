...
def _archive_metadata_is_selectable(metadata: object) -> bool:
...
def _require_archive_selectable(
    snapshot_name: str, container: ContainerClient | None = None
) -> None:
    container = container or _container_client()
    snapshot_stem = _snapshot_stem(snapshot_name)
    try:
        properties = container.get_blob_client(
            _archive_blob_name(snapshot_stem)
        ).get_blob_properties()
    except ResourceNotFoundError as exc:
        raise BackupSnapshotNotFoundError(snapshot_stem) from exc
    if not _archive_metadata_is_selectable(getattr(properties, "metadata", None)):
        raise BackupSnapshotNotFoundError(snapshot_stem)
...
def get_backup_observability_state() -> dict | None:
    ...
    try:
        ...
        logger.warning(
            "Failed to parse backup observability state; falling back to legacy marker"
        )
        state = None
...
def list_retained_backup_archives() -> dict[str, dict[str, datetime | int | None]]:
    ...
    for blob in container.list_blobs(name_starts_with=prefix, include=["metadata"]):
        if not blob.name.endswith(".tar.gz") or not _archive_metadata_is_selectable(
        ...
        ) or _created_at_from_snapshot_name(name)
...
def get_snapshot_manifest(snapshot_name: str) -> dict:
    ...
    snapshot_stem = _snapshot_stem(snapshot_name)
    container = _container_client()
    _require_archive_selectable(snapshot_stem, container)
    ...
    try:
        ...
        if isinstance(manifest, dict):
            return manifest
        raise BackupSnapshotManifestError(
            f"Manifest sidecar {sidecar_blob} did not contain a JSON object"
        )
    except ResourceNotFoundError:
        pass
        ...
        with tarfile.open(fileobj=reader, mode="r|gz") as tar:
            for member in tar:
                ...
                try:
                    ...
                    Exception
                ) as exc:  # noqa: BLE001 - surface manifest parse issues cleanly
                    raise BackupSnapshotManifestError(
                        f"Failed to parse {member_name}"
                    ) from exc
                if not isinstance(manifest, dict):
                    raise BackupSnapshotManifestError(
                        f"{member_name} did not contain a JSON object"
                    )
                return manifest
    except ResourceNotFoundError as exc:
        raise BackupSnapshotNotFoundError(snapshot_stem) from exc
    ...
    raise BackupSnapshotManifestError(
        f"Manifest {member_name} was not found in snapshot {snapshot_stem}"
    )
...
def _manifest_file_entry(manifest: dict, member_path: str) -> dict:
    ...
    if not isinstance(entry, dict):
        raise BackupSnapshotMemberError(
            f"{member_path} is not listed in the snapshot manifest"
        )
    return entry
...
def restore_snapshot_file(
    ...
    *member_path* (e.g. cached at enqueue time); when provided the manifest
    ...
    if not isinstance(expected_sha256, str) or not expected_sha256:
        raise BackupSnapshotManifestError(
            f"Manifest entry for {member_path} is missing sha256"
        )
    if not isinstance(expected_size, int):
        raise BackupSnapshotManifestError(
            f"Manifest entry for {member_path} is missing size"
        )
            ...
            with tarfile.open(fileobj=reader, mode="r|gz") as tar:
                for member in tar:
                    ...
                    if not member.isfile() or member.islnk() or member.issym():
                        raise BackupSnapshotMemberError(
                            f"{member_path} is not a regular file"
                        )
                    extracted = tar.extractfile(member)
                    if extracted is None:
                    ...
                    raise BackupSnapshotMemberError(
                        f"{member_path} was not found in snapshot {snapshot_stem}"
                    )
        except Exception:
            try:
                tmp_path.unlink()
