...
| ------------------------------------ | ------------------------------------ | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `TASK_EXECUTION_MODE`                | `local`                              | `local` permits in-process fallback when Redis is unavailable; `required` rejects queue-backed work with HTTP 503 instead. Also selects where the reconciliation sweep runs: once at API startup in `local` mode, or periodically via an arq cron job on the worker in `required` mode (see `app/reconciliation.py`).                                                                                |
| `WORKER_MAX_JOBS`                    | `4`                                  | Maximum concurrent arq jobs per dedicated worker (minimum `2`); also bounds in-process vips concurrency during local-mode fallback. Bulk-import coordinators are API-hosted orchestration tasks, so they do not consume arq worker slots while child jobs run.                                                                                                                                       |
| `REBUILD_PARALLEL_ENABLED`           | `false`                              | Gates creation of durable parallel tile rebuild jobs. Wave 1 exposes no public parallel creation API; the serial admin endpoint and automatic post-import rebuild remain unchanged.                                                                                                                                                                                                                  |
| `REBUILD_PARALLELISM`                | `2`                                  | Independent maximum number of durable tile-rebuild children in `running` state. PostgreSQL counts this window; it does not inherit `WORKER_MAX_JOBS`.                                                                                                                                                                                                                                                |
| `REBUILD_CHILD_TIMEOUT_SECONDS`      | `3600`                               | Per-image durable rebuild child timeout. The 2026-09-18 repeat rehearsal exhausted the prior 1,800-second default on representative large sources. Must be shorter than `REBUILD_LEASE_SECONDS`.                                                                                                                                                                                                     |
| `REBUILD_LEASE_SECONDS`              | `3900`                               | Durable rebuild claim lease. The 300-second margin above the child timeout preserves authority throughout a healthy attempt; expired ownership is reclaimed and repumped from PostgreSQL.                                                                                                                                                                                                            |
| `REBUILD_HEARTBEAT_SECONDS`          | `30`                                 | Lease-renewal cadence for a running durable rebuild child. Must be shorter than the lease.                                                                                                                                                                                                                                                                                                           |
| `REBUILD_PUMP_CADENCE_SECONDS`       | `60`                                 | Periodic worker backstop for lost pump triggers and commit-before-enqueue gaps; must be a whole-minute interval between 60 and 3600 seconds.                                                                                                                                                                                                                                                         |
| `REBUILD_MAX_ATTEMPTS`               | `2`                                  | Maximum claim-time attempts for automatically retried durable rebuild items.                                                                                                                                                                                                                                                                                                                         |
| `REBUILD_RETRY_BACKOFF_BASE_SECONDS` | `60`                                 | Initial PostgreSQL-persisted delay before a transient durable rebuild retry becomes claimable.                                                                                                                                                                                                                                                                                                       |
| `REBUILD_RETRY_BACKOFF_CAP_SECONDS`  | `900`                                | Maximum persisted exponential delay between durable rebuild attempts; must be at least the base delay.                                                                                                                                                                                                                                                                                               |
| `VIPS_CONCURRENCY`                   | unset (libvips detects host CPUs)    | libvips per-operation thread-pool size. Rendered on both the API pod (`api.vipsConcurrency`, default `1`) and the dedicated worker deployment (`redis.worker.vipsConcurrency`, default `1`) so libvips respects the pod's cgroup CPU limit instead of oversubscribing it across concurrent jobs. On the API it only matters in `local` mode, where the in-process fallback can run image processing. |
...
| `TILE_TOKEN_TTL_MINUTES` | `15`    | Lifetime of image-scoped tile tokens appended to `tile_sources`/`thumb` URLs in API responses (bounded 1–1440). Signed with `JWT_SECRET`. See `docs/tile-delivery-boundary.md`. |
