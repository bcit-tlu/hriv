...
| `VIPS_CONCURRENCY`                   | unset (libvips detects host CPUs)    | libvips per-operation thread-pool size. Set on the dedicated worker deployment (`redis.worker.vipsConcurrency` in the Helm chart, default `1`) so libvips respects the worker pod's cgroup CPU limit instead of oversubscribing it across `WORKER_MAX_JOBS` concurrent jobs.                                          |
