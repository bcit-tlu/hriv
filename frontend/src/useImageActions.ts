...
export function useImageActions(deps: UseImageActionsDeps) {
    ...
    const handleSaveViewerImage = useCallback(
        ...
        [selectedImage, refreshCategories, loadUncategorizedImages, setSelectedImage, setPath, setErrorSnack],
    );
    ...
    const handleReplaceViewerImage = useCallback(
        ...
        [selectedImage, loadCategories, loadUncategorizedImages, startReplaceUpload, trackReplaceProgress, transitionReplaceToProcessing, removeReplaceUpload, failReplaceUpload, setSelectedImage],
    );
    ...
    const handleDeleteViewerImage = useCallback(
        async () => {
            if (!selectedImage) return;
    ...
    const handleDeleteBrowseImage = useCallback(
        async () => {
            if (!browseEditImage) return;
