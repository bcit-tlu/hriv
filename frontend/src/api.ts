...
function readStoredToken(): string | null {
    try {
        return getLocalStorage()?.getItem("hriv_token") ?? null;
    } catch {
        return null;
    }
}
...
export function fetchUncategorizedImages(
    init?: RequestInit,
): Promise<ApiImage[]> {
...
export async function fetchUsersPaged(
    ...
    for (const id of params.programIds ?? [])
        qs.append("program_id", String(id));
...
export function fetchGroupInstructors(
    groupId: number,
): Promise<ApiGroupMember[]> {
    return request(`/groups/${groupId}/instructors`);
}

export function addGroupInstructor(
    groupId: number,
    userId: number,
): Promise<ApiGroup> {
...
export function removeGroupInstructor(
    groupId: number,
    userId: number,
): Promise<ApiGroup> {
    return request(`/groups/${groupId}/instructors/${userId}`, {
        ...
        xhr.addEventListener("load", () => {
            try {
                ...
                reject(
                    e instanceof Error
                        ? e
                        : new Error("Failed to parse replace response"),
                );
            }
        });
        ...
        xhr.addEventListener("load", () => {
            try {
                ...
                reject(
                    e instanceof Error
                        ? e
                        : new Error("Failed to parse bulk import response"),
                );
            }
        });
