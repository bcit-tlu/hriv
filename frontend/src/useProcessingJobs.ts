...
export function useProcessingJobs(deps: UseProcessingJobsDeps) {
  ...
  const bulkImportPollRefs = useRef<Map<number, ReturnType<typeof setInterval>>>(new Map())
  ...
  const getVisibleJobs = useCallback(
    (filter: VisibleJobsFilter): ProcessingJob[] => {
      return processingJobs.filter((j) => {
        if (!(j.status === 'uploading' || j.status === 'failed') || j.uploadId == null) return true
        const isReplaceJob = j.uploadId < 1_000_000_000
        if (isReplaceJob) return !(filter.imageEditOpen || filter.browseEditImage != null)
    ...
    for (const job of processingJobs) {
      ...
      const handle = pollProcessingJob(job.id, {
        ...
        onProgress: (progress, statusMessage) => {
          ...
          if (statusMessage) {
            serverStatusMessageRef.current.set(job.id, statusMessage)
          }
        },
      })
  ...
  const updateBulkImportJob = useCallback(
    (bulkJob: ApiBulkImportJob, filename: string, fileSize: number, uploadId?: number) => {
      const done = bulkJob.completed_count + bulkJob.failed_count
      const progress = bulkJob.total_count > 0 ? Math.round((done / bulkJob.total_count) * 100) : 0
      setProcessingJobs((prev) => {
        const existing = prev.find(
          (j) =>
            j.bulkImportJobId === bulkJob.id || (uploadId !== undefined && j.uploadId === uploadId),
        )
        const status =
        ...
        if (
          prev.filter(
            (j) =>
              j.status === 'uploading' || j.status === 'processing' || j.status === 'importing',
  ...
  useEffect(() => {
    ...
    for (const [id, interval] of refs) {
      if (!processingJobs.some((j) => j.bulkImportJobId === id && j.status === 'importing')) {
        clearInterval(interval)
        refs.delete(id)
      }
    }
  ...
  useEffect(() => {
    const hasActiveJob = processingJobs.some(
      (j) => j.status === 'processing' || j.status === 'uploading' || j.status === 'importing',
    )
  ...
  const handleUploadProgress = useCallback((uploadId: number, fraction: number) => {
  ...
  const handleProcessingStarted = useCallback(
    (sourceImageId: number, filename: string, fileSize: number, uploadId: number) => {
      setProcessingJobs((prev) => {
        const uploadingJob = prev.find((j) => j.status === 'uploading' && j.uploadId === uploadId)
        if (uploadingJob) {
  ...
  const handleBulkImportStarted = useCallback(
    (job: ApiBulkImportJob, filename: string, fileSize: number, uploadId: number) => {
      uploadProgressRef.current.delete(uploadId)
  ...
  const startReplaceUpload = useCallback(
    (file: File, context: 'viewer' | 'browse'): { uploadId: number; abort: AbortController } => {
  ...
  const trackReplaceProgress = useCallback((uploadId: number, fraction: number) => {
    uploadProgressRef.current.set(uploadId, fraction)
  }, [])

  /** Transition a replacement upload to the processing phase. */
  const transitionReplaceToProcessing = useCallback((uploadId: number, sourceImageId: number) => {
  ...
  const failReplaceUpload = useCallback((uploadId: number, errorMessage: string) => {
    replaceAbortRef.current = null
    uploadProgressRef.current.delete(uploadId)
    activeReplaceUploadIdRef.current = null
    setProcessingJobs((prev) =>
  ...
  const removeReplaceUpload = useCallback((uploadId: number) => {
    ...
    uploadProgressRef.current.delete(uploadId)
    activeReplaceUploadIdRef.current = null
    setProcessingJobs((prev) => prev.filter((j) => j.uploadId !== uploadId))
  }, [])
