...
export default function AuthProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    const stored = sessionStorage.getItem('corgi_user')
    ...
    if (stored) {
      try {
        ...
        setToken(null)
        sessionStorage.removeItem('corgi_user')
        setLoading(false)
        return
      }
    }
    ...
    fetch(`${import.meta.env.VITE_API_URL ?? ''}/api/auth/me`, {
      ...
      .then(async (res) => {
        if (res.ok) {
          ...
          sessionStorage.setItem('corgi_user', JSON.stringify(freshUser))
        } else {
  ...
  const login = useCallback(
    async (email: string, password: string) => {
      ...
      setCurrentUser(user)
      sessionStorage.setItem('corgi_user', JSON.stringify(user))
    },
    [],
  )
