...
function buildCategoryPaths(
    ...
    for (const node of nodes) {
        ...
        const childMap = buildCategoryPaths(node.children, [
            ...ancestors,
            node,
        ]);
...
const MANAGE_ALL_COLUMNS: readonly ManageTableColumn[] =
    MANAGE_COLUMN_OPTIONS.map((column) => column.key);
...
export default function ManagePage({
    ...
    const [replaceProgress, setReplaceProgress] = useState<number | undefined>(
        undefined,
    );
    ...
    const [prevInitialProgramFilter, setPrevInitialProgramFilter] =
        useState(initialProgramFilter);
    ...
    const [deleteDialogImage, setDeleteDialogImage] = useState<ApiImage | null>(
        null,
    );
    ...
    const getInheritedProgramIds = useCallback(
        (img: ApiImage): { direct: number[]; ancestor: number[] } => {
            ...
            return splitDirectAncestorProgramIds([
                ...seg.ancestors,
                seg.category,
            ]);
        },
    ...
    const getInheritedGroupIds = useCallback(
        (img: ApiImage): { direct: number[]; ancestor: number[] } => {
            ...
            const seg = categoryPaths.get(img.category_id);
            if (!seg) return { direct: [], ancestor: [] };
            return splitDirectAncestorGroupIds([
                ...seg.ancestors,
                seg.category,
            ]);
        },
        ...
        sorted.sort((a, b) => {
            ...
            switch (sortColumn) {
                ...
                case "name":
                    ...
                    break;
                case "category":
                    cmp = getCategoryLabel(a).localeCompare(
                        getCategoryLabel(b),
                    );
                    break;
                case "copyright":
    ...
    const maxPage = Math.max(
        0,
        Math.ceil(sortedImages.length / rowsPerPage) - 1,
    );
                            ...
                            <TableRow>
                                <TableCell padding="checkbox">
                                    <Checkbox
                                        indeterminate={
                                            selectedInView > 0 &&
                                            selectedInView < pageImages.length
                                        }
                                        checked={
                                            pageImages.length > 0 &&
                                            selectedInView === pageImages.length
            ...
            >
                ...
                >
                    ...
                    >
                        ...
                        <strong>{deleteDialogImage?.name}</strong>.
                    </Typography>
                    <Typography variant="body2" color="text.secondary">
                        To delete multiple images at once, select them and use
                        the <strong>Bulk Edit</strong> option.
                    </Typography>
                    <Divider />
                    <Box>
                        <Button
