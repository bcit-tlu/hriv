...
export default function EditCategoryDialog({
  ...
  useEffect(() => {
    if (open && !prevOpen.current) {
      ...
      setError(null)
      setVisibility(
        currentProgramIds.length > 0 || inheritedProgramIds.length > 0 ? 'specific' : 'all',
      )
      ...
      setGroupVisibility(
        currentGroupIds.length > 0 || inheritedGroupIds.length > 0 ? 'specific' : 'all',
      )
      const validGroupIds =
        inheritedGroupIds.length > 0
    ...
    <Dialog open={open} onClose={handleClose} maxWidth="xs" fullWidth>
      ...
      <DialogActions>
        ...
        <Button
          ...
          disabled={
            !label.trim() ||
            (!labelChanged && !programsChanged && !groupsChanged && !statusChanged) ||
            (visibility === 'specific' &&
