...
describe('useShareableImageState', () => {
  ...
  describe('clearImage', () => {
    it('calls setSelectedImage(null) and resets viewport/overlays', () => {
      ...
      const { result } = renderHook(() => useShareableImageState(makeDeps({ setSelectedImage })))
      ...
      act(() => {
        ...
        result.current.handleOverlaysChange([{ x: 0, y: 0, w: 1, h: 1 }])
      })
      act(() => {
  ...
  describe('lockedOverlays', () => {
    ...
    it('derives locked overlays from selectedImage metadata', () => {
      ...
      const { result } = renderHook(() => useShareableImageState(makeDeps({ selectedImage: img })))
      expect(result.current.lockedOverlays).toEqual([{ x: 0.1, y: 0.2, w: 0.3, h: 0.4 }])
  ...
  describe('URL parsing on mount', () => {
    it('parses image ID and viewport from URL', () => {
      window.history.replaceState(null, '', '/?image=42&zoom=2.5&x=0.3&y=0.7')
      const setSelectedImage = vi.fn()
    ...
    it('parses overlay rectangles from URL', () => {
      window.history.replaceState(null, '', '/?image=42&ov0=0.1,0.2,0.3,0.4')
      const setSelectedImage = vi.fn()
      const img = makeImage({ id: 42 })
      const cat = makeCategory({ id: 1, label: 'Root', images: [img] })
      const { result } = renderHook(() =>
      ...
      renderHook(() =>
        useShareableImageState(
          makeDeps({
            ...
            categories: [],
            uncategorizedImages: [],
            uncategorizedLoaded: uncategorizedLoaded as React.RefObject<boolean>,
          }),
        ),
      )
  ...
  describe('initialViewport', () => {
    it('is stable across re-renders for the same image', () => {
      ...
      const { result, rerender } = renderHook(
        (props: UseShareableImageStateDeps) => useShareableImageState(props),
  ...
  describe('initialOverlays', () => {
    it('uses locked overlays when no URL overlays present', () => {
      ...
      const { result } = renderHook(() => useShareableImageState(makeDeps({ selectedImage: img })))
      expect(result.current.initialOverlays).toEqual([{ x: 0, y: 0, w: 0.5, h: 0.5 }])
    })
  })
  ...
  describe('clearPending', () => {
    it('clears pendingImageId so URL sync resumes', () => {
      window.history.replaceState(null, '', '/?image=999')
      const { result } = renderHook(() =>
        useShareableImageState(makeDeps({ categoriesLoading: true })),
      )
  ...
  describe('rotation parsing', () => {
    it('parses rotation from URL', () => {
      window.history.replaceState(null, '', '/?image=42&zoom=1&x=0.5&y=0.5&rotation=90.0')
      const setSelectedImage = vi.fn()
      const img = makeImage({ id: 42 })
      const cat = makeCategory({ id: 1, label: 'Root', images: [img] })
      renderHook(() =>
