...
describe('FileDropZone', () => {
  describe('visibility', () => {
    it('returns null when isDragActive is false', () => {
      const { container } = renderWithTheme(<FileDropZone isDragActive={false} onDrop={vi.fn()} />)
      expect(container.firstChild).toBeNull()
    })

    it('renders the drop zone when isDragActive is true', () => {
      renderWithTheme(<FileDropZone isDragActive={true} onDrop={vi.fn()} />)
      expect(
  ...
  describe('drop behaviour', () => {
    it('calls onDrop with dropped files', () => {
      const onDrop = vi.fn()
      renderWithTheme(<FileDropZone isDragActive={true} onDrop={onDrop} />)
