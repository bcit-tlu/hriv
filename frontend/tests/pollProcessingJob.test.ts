...
describe('pollProcessingJob', () => {
  ...
  it('calls onCompleted with image_id when backend reports completed', async () => {
    const fetchStatus = vi.fn().mockResolvedValue(makeStatus({ status: 'completed', image_id: 42 }))
  ...
  it('passes null to onCompleted when image_id is missing', async () => {
    const fetchStatus = vi.fn().mockResolvedValue(makeStatus({ status: 'completed' }))
    const onCompleted = vi.fn()
  ...
  it('passes null error message when backend omits it', async () => {
    const fetchStatus = vi.fn().mockResolvedValue(makeStatus({ status: 'failed', progress: 10 }))
    const onFailed = vi.fn()
  ...
  it('calls onProgress and reschedules another poll when status is non-terminal', async () => {
    const fetchStatus = vi
      ...
      .mockResolvedValueOnce(makeStatus({ status: 'completed', image_id: 99 }))
    const onCompleted = vi.fn()
    const onProgress = vi.fn()
  ...
  it('treats unknown / pending backend status as progress and keeps polling', async () => {
    const fetchStatus = vi
      .fn()
      .mockResolvedValueOnce(makeStatus({ status: 'pending', progress: 0 }))
      .mockResolvedValueOnce(makeStatus({ status: 'something-unexpected', progress: 5 }))
      .mockResolvedValueOnce(makeStatus({ status: 'completed', image_id: 1 }))
    const onProgress = vi.fn()
    const onCompleted = vi.fn()
  ...
  it('retries after fetch rejection using the configured interval', async () => {
    const fetchStatus = vi
      ...
      .mockResolvedValueOnce(makeStatus({ status: 'completed', image_id: 7 }))
    const onCompleted = vi.fn()
  ...
  it('cancel() is idempotent (safe to call multiple times)', async () => {
    const fetchStatus = vi.fn().mockResolvedValue(makeStatus({ status: 'processing', progress: 1 }))
  ...
  it('handles multiple concurrent jobs independently', async () => {
    const fetchA = vi.fn().mockResolvedValue(makeStatus({ status: 'completed', image_id: 100 }))
    const fetchB = vi.fn().mockResolvedValue(makeStatus({ status: 'failed', progress: 20 }))
  ...
  it('retries the poll when onCompleted rejects (transient refresh failure)', async () => {
    ...
    const fetchStatus = vi.fn().mockResolvedValue(makeStatus({ status: 'completed', image_id: 1 }))
