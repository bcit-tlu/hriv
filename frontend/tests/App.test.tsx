...
let mockInitialPath: typeof mockDeepPath = [];
...
vi.mock("../src/components/MoveCategoryDialog", () => ({
    default: () => null,
}));
vi.mock("../src/components/AddCategoryDialog", () => ({ default: () => null }));
vi.mock("../src/components/EditCategoryDialog", () => ({
    default: () => null,
}));
vi.mock("../src/components/AddEditPersonModal", () => ({
    default: () => null,
}));
...
vi.mock("../src/useShareableImageState", () => ({
    useShareableImageState: ({
        ...
        setPath: (path: typeof mockDeepPath) => void;
    }) => {
        useEffect(() => {
...
describe("App breadcrumbs", () => {
    ...
    it("renders program and group chips in both browse and image breadcrumb rows", () => {
        ...
        expect(
            within(categoryBreadcrumb as HTMLElement).getByText("Pathology"),
        ).toBeInTheDocument();
        expect(
            within(categoryBreadcrumb as HTMLElement).getByText("Lab A2"),
        ).toBeInTheDocument();
        ...
        expect(
            within(imageBreadcrumb as HTMLElement).getByText("Pathology"),
        ).toBeInTheDocument();
        expect(
            within(imageBreadcrumb as HTMLElement).getByText("Lab A2"),
        ).toBeInTheDocument();
    });
    ...
    it("collapses deep category breadcrumbs to the parent and current category", async () => {
        ...
        const categoryBreadcrumb = await screen.findByLabelText(
            "category breadcrumb",
        );
        expect(within(categoryBreadcrumb).getByText("...")).toBeInTheDocument();
        expect(
            within(categoryBreadcrumb).queryByText("Human Anatomy"),
        ).not.toBeInTheDocument();
        expect(
            within(categoryBreadcrumb).getByText("Head Pathologies"),
        ).toBeInTheDocument();
        expect(
            within(categoryBreadcrumb).getByText(
                "Intracranial Hemorrhages ICH 1",
            ),
        ).toBeInTheDocument();
        expect(
            within(categoryBreadcrumb).getByLabelText(
                "Skipped categories: Human Anatomy",
            ),
        ).toBeInTheDocument();
    });
    ...
    it("collapses deep image breadcrumbs to the direct image category and image", async () => {
        ...
        expect(
            within(imageBreadcrumb).queryByText("Human Anatomy"),
        ).not.toBeInTheDocument();
        expect(
            within(imageBreadcrumb).queryByText("Head Pathologies"),
        ).not.toBeInTheDocument();
        expect(
            within(imageBreadcrumb).getByText("Intracranial Hemorrhages ICH 1"),
        ).toBeInTheDocument();
        expect(
            within(imageBreadcrumb).getByText("Specimen Image"),
        ).toBeInTheDocument();
        expect(
