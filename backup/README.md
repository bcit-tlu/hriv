...
result to stdout and exits nonzero with an exact `{schema_version,operation,success:false,failure_code,failure_stage}` document on failure. Selection and stateless restore also bind `wal_segment_size_bytes` so the WAL fence filename can be validated for non-default PostgreSQL segment sizes. The manifest sidecar is
