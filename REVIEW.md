...
- **Tile move vs. reorder is a locked contract — see `docs/drag-and-drop.md`.** The Browse grid (`SortableTileGrid.tsx`) uses `@dnd-kit/react` v2. Move-into-category owns a **centered inset region** of the category tile (`DroppableCategoryZone`, `pointerIntersection`, `CollisionPriority.High`, inset `MOVE_ZONE_INSET_PX`) and always wins there. Reorder uses **optimistic `useSortable` reflow (A2)**: tiles reflow live and a drop commits the reflowed order via the `move()` helper. Reflow is suppressed over the move zone by the sortable's `collisionDetector` (`createGapOnlyClosestCenter`), which returns `null` while the pointer is inside a registered move-zone rect — so move wins in a category tile's centre, while the inset margin + inter-tile gap stay reorder-active (keeps category-to-category reorder practical). The move collision boundary and the suppression boundary are the **same inset element**, so the live preview matches what commits. (A2 superseded A1's gap-only seam zones; see the Decision Record.) Changing collision detection, zones, priority, the inset, or activation requires updating that doc in the same PR and a human feel-test before merge (scripted/recorded drags cannot prove feel).
