...
- **Tile move vs. reorder is a locked contract — see `docs/drag-and-drop.md`.** The Browse grid (`SortableTileGrid.tsx`) uses `@dnd-kit/react` v2. Move-into-category owns the **full category-tile rect** (`DroppableCategoryZone`, `CollisionPriority.High`). Reorder uses **optimistic `useSortable` reflow (A2)**: tiles reflow live and a drop commits the reflowed order via the `move()` helper. The move-wins guard is a **directional far-half threshold** shared by two complementary collision detectors keyed on `isPastTileCenterAlongDrag(pointer, center, delta)` (cumulative drag delta, not the jittery `position.direction`): `farHalfReorderCollision` (sortables) only reorders once the pointer crosses a tile's centre on the side opposite the entry edge; `nearHalfMoveCollision` (move zone, High) only fires on the near half. So a tile's near/entry half is move ("Move here" over a category) or a dead-zone (image), and the far half reorders — keeping category↔category reorder possible (push past centre) without an aim-for-the-gap target. (Supersedes the broken inset move zone + `createGapOnlyClosestCenter`, and A1's gap-only seam zones; see the Decision Records.) Changing collision detection, zones, priority, the centre threshold, or activation requires updating that doc in the same PR and a human feel-test before merge (scripted/recorded drags cannot prove feel).
